...
public class Main {
    ...
    public static void main(String[] args) throws InvalidProtocolBufferException {
        // 连接远程 RPC
        Client client = new Client("127.0.0.1", 10086, false);
        // 本地启动 RPC
//        Client client = new Client(); // 默认 10086 端口
//         Client client = new Client(10088,true); // 也可以指定端口
        // 是否已登录

//        List<Wcf.DbRow> dbRows = client.querySql("MicroMsg.db", "SELECT UserName, NickName FROM Contact;");
//        for (Wcf.DbRow dbRow : dbRows) {
//            for (Wcf.DbField dbField : dbRow.getFieldsList()) {
//                System.out.println("dbField.getColumn() = " + dbField.getColumn());
//                System.out.println("dbField.getContent().toStringUtf8()"+ dbField.getContent().toStringUtf8());
//            }
//        }

        List<Wcf.DbRow> dbRows = client.querySql("MicroMsg.db", "SELECT RoomData FROM ChatRoom WHERE ChatRoomName = '52876223693@chatroom';");
        for (Wcf.DbRow dbRow : dbRows) {
            for (Wcf.DbField dbField : dbRow.getFieldsList()) {
                System.out.println("dbField.getColumn() = " + dbField.getColumn());
                Roomdata.RoomData roomData = Roomdata.RoomData.parseFrom(dbField.getContent().toByteArray());
                List<Roomdata.RoomData.RoomMember> membersList = roomData.getMembersList();
                for (Roomdata.RoomData.RoomMember roomMember : membersList) {
                    System.out.println("roomMember.getName()+roomMember.getWxid() = " + roomMember.getName() + "====" + roomMember.getWxid()+"====="+roomMember.getState());
                }
            }
        }
...
//        ByteString content = dbRows.get(0).getFields(0).getContent();
//
//        Map<Descriptors.FieldDescriptor, Object> allFields = dbRows.get(0).getFields(0).getAllFields();
//        System.out.println("allFields = " + allFields);
//        logger.info("isValidUtf8 = " + content.isValidUtf8());
//        logger.info("content = " + content.toStringUtf8());
//        String column = dbRows.get(0).getFields(0).getColumn();
//        logger.info("column = " + column);
//
//        logger.info("dbRows = " + dbRows);
...
//        client.sendText("Hello", "HV1551541819", "");
//
        ...
        Thread thread = new Thread(new Runnable() {
            public void run() {
                while (client.getIsReceivingMsg()) {
                    ...
                    client.printWxMsg(msg);
                    String content = msg.getContent();
                    System.out.println("content = " + content);
                    client.sendText(msg.getContent() + "ok", msg.getRoomid(), "");
                }
            }
        });
        thread.start();
