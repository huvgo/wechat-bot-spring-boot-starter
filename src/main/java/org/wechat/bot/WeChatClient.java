...
import org.wechat.bot.core.Receiver;
import org.wechat.bot.core.SDK;
import org.wechat.bot.core.Wcf;
import org.wechat.bot.core.Wcf.*;
...
public class WeChatClient {
    ...
    public WeChatClient(String host, int port, boolean debug) {
        this.host = host;
        this.port = port;
        int status = SDK.INSTANCE.WxInitSDK(debug, port);
        if (status != 0) {
    ...
    public List<RpcContact> getFriends() {
        List<RpcContact> friends = new ArrayList<>();
        for (RpcContact contact : getContacts()) {
            if ("好友".equals(contactType(contact.getWxid()))) {
                friends.add(contact);
            }
        }
        return friends;
    ...
    public List<RpcContact> getChatRooms() {
        List<RpcContact> chatRooms = new ArrayList<>();
        for (RpcContact contact : getContacts()) {
    ...
    // return string 类型
    private String contactType(String wxid) {
        Map<String, String> notFriends = Map.of("fmessage", "朋友推荐消息", "filehelper", "文件传输助手", "floatbottle", "漂流瓶", "medianote", "语音记事本", "mphelper", "公众平台助手", "newsapp", "新闻");
        if (notFriends.get(wxid) != null) {
            return notFriends.get(wxid);
        } else if (wxid.endsWith("@chatroom")) {
            return "群聊";
        } else if (wxid.endsWith("@openim")) {
            return "企业微信";
        } else if (wxid.startsWith("gh_")) {
            return "公众号";
        }
        return "好友";
    }

    public void receiveMsg(Receiver receiver) {
        enableRecvMsg(100);
        Thread thread = new Thread(() -> {
            while (getIsReceivingMsg()) {
                receiver.receive(getMsg());
            }
        });
        thread.start();
    }
